...
abstract public class AbstractDemo implements Demo, ProductNames {
	public static JFrame frame; //added
	public static MyMenu menu; //added
	...
	public void setAutoRefresh(boolean status){
		if(status){
			menu.refreshOff.setSelected(false);
			menu.refreshOn.setSelected(true);
		}
		else{
			menu.refreshOn.setSelected(false);
			menu.refreshOff.setSelected(true);
		}
	}
    ...
    //Same as showAsFrame except demo.getDemoPanel() executed first
    public static JFrame repaintFrame(final Demo demo){
    	//Do time consuming demoPanel creation first
    	frame.setTitle("Refreshing...");
    	menu = new MyMenu(demo);
    	JMenuBar mbar = menu.createMenuBar();
    	Component demoPanel = demo.getDemoPanel();
    	
    	//Hide old frame
    	frame.setVisible(false);
    	//final JFrame 
        frame = new JFrame(demo.getName());
        
      //Get size and location of frame
        ...
        //frame.setContentPane(mdemo.createContentPane());
        ...
        //Uncomment to add Options Panel
...
//        com.jidesoft.swing.JideSwingUtilities.traceFocus();
    ...
    //Creates the Options panel, called with a JFrame, Demo and Component
    protected static JComponent createOptionsPanel(JFrame parentFrame, Demo demo, Component demoPanel) {
        ...
        if (demoPanel != null) {
            ...
            if (source != null && source.length > 0) {
                ...
                //Displays source in Source Code panel
    ...
    private static JComponent createCommonOptions(final Component component) {
        ...
        //Added the Object argument for Comparator
    ...
    //Returns the name/location of the source code file
    public String[] getDemoSource() {    	
        //This is for determining file names on the fly (not perfected)
    ...
    //Browse Source Code button, called with a JFrame and a Demo
...
//Panel for source code, called with an array of source code files
...
//    }
    ...
    //After clicking on Get Source Code, called with a source code file name
...
//        area.setFont(new Font("Monospaced", Font.PLAIN, 12));
        ...
        try {
        	...
        	//in1 tries to get the file name by looking at the classes
            //InputStream in1 = AbstractDemo.class.getResourceAsStream(fileName);
        	
            //in2 uses the hard-coded file name(s)
            InputStream in2 = new FileInputStream(fileName);
            if (in2 != null) {
                byte[] buff = new byte[4096];
                int nch;
                while ((nch = in2.read(buff, 0, buff.length)) != -1) {
